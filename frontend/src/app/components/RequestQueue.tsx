        ...
        <div className="queue-list" id="queue-list">
          {requests.length === 0 ? (
            ...
            requests.slice(0, 10).map((request, index) => {
              ...
              const priority =
                waitTime > 30 ? "urgent" : waitTime > 15 ? "priority" : "";
              const assignedText = request.assigned
                ? ` (E${request.elevatorId})`
                : "";

              return (
                <div className={`queue-item ${priority}`} key={index}>
                  <div className="request-info">
                    <span className="request-floors">
                      {request.originFloor} → {request.destinationFloor}
                      {assignedText}
                    </span>
